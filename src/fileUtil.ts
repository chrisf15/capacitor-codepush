...
export class FileUtil {
    public static async directoryExists(directory: Directory, path: string): Promise<boolean> {
        try {
            ...
            // directory for Android, NSFileTypeDirectory for iOS on Capacitor 3
            // TODO(cap3): Remove `NSFileTypeDirectory` check when dropping support for Capacitor 3
            return statResult.type === "directory" || (statResult.type as string) === "NSFileTypeDirectory";
        } catch (error) {
            return false;
        }
    }
    ...
    public static async fileExists(directory: Directory, path: string): Promise<boolean> {
        try {
            const statResult = await Filesystem.stat({directory, path});
            // file for Android, NSFileTypeRegular for iOS on Capacitor 3
            // TODO(cap3): Remove `NSFileTypeRegular` check when dropping support for Capacitor 3
            return statResult.type === "file" || (statResult.type as string) === "NSFileTypeRegular";
        } catch (error) {
            return false;
        }
    }
        ...
        */
        ...
        if (await FileUtil.directoryExists(destinationDir.directory, destinationDir.path)) {
            ...
            for (const fileInfo of files) {
                // TODO(cap3): Remove `string` runtime check when dropping support for Capacitor 3
